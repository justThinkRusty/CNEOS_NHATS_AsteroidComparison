...
# Adjust the force_tty field if teletype not supported in your environment
with alive_bar(405, spinner = 'wait3', bar = 'filling', title = 'NHATS API Pulls', force_tty = True, stats = True, monitor = True) as bar:  # Progress bar initiation - bar = 'classic3' also good
...
print("Total # of NEAs in NHATS Database: " + str(NHATS_tot))
...
print("Total known NEAs: " + str(Total_NEAs))  # Nice print - not necessary

frac_NEAs = NHATS_tot/Total_NEAs*100; perc = round(frac_NEAs, 2)  # What fraction of NEAs are in NHATS
print("Percentage of all known small bodies: " + str(perc) + "%")  # Nice print - not necessary
...
# If the string comparison did not find a direct correlation for all the NHATS bodies
if sum_test != NHATS_tot:
    print("Houston, we have a problem")  # Issue
else:
    print("Houston, we found them all")  # Humor
...
background.show() #Unnecessary show function but is nice
...
print("Time to run: " + str(round(time.time() - t, 2)))  # Total time tracking for curioisity sake
